...
void CLogger::logMessage(int level, const char *file, int line, const char *function, const char *fmt, ...) {
    ...
    //每分钟从 ${logfile}.cfg 中更新日志级别 --tomzhou
    if (_lastMin != tm.tm_min && _name != NULL) {
        char * _lvlFile;
        _lvlFile = new char[strlen(_name) + 5];
        _lvlFile[0] = '\0';
        strcat(_lvlFile, _name);
        strcat(_lvlFile, ".cfg");

        FILE * tmpFp;
        char tmpLevel[10];
        tmpLevel[0] = '\0';
        tmpFp = fopen(_lvlFile, "r");
        if (tmpFp != NULL) {
            fgets(tmpLevel, 10, tmpFp);
            fclose(tmpFp);

            int i = strlen(tmpLevel) - 1;
            while (i >= 0 && (tmpLevel[i] == '\r' || tmpLevel[i] == '\n' || tmpLevel[i] == ' ' || tmpLevel[i] == '\t')) {
                tmpLevel[i] = '\0';
                i--;
            }
    ...
    if (level < SYS_LOG_LEVEL_INFO) {
        size = snprintf(buffer, 5000, "[%04d-%02d-%02d %02d:%02d:%02d] [%s] %s (%s:%d) %s\n", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, _errstr[level], function,
                file, line, data1);
    } else {
        size = snprintf(buffer, 5000, "[%04d-%02d-%02d %02d:%02d:%02d] [%s] %s:%d %s\n", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, _errstr[level], file, line,
                data1);
    }
    // 去掉过多的换行
    while (buffer[size - 2] == '\n')
...
void CLogger::rotateLog(const char *filename, const char *fmt) {
    ...
    if (access(filename, R_OK) == 0) {
        ...
        if (fmt != NULL) {
            ...
            sprintf(oldLogFile, "%s.%04d%02d%02d%02d%02d%02d", filename, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
        }
        if (_maxFileIndex > 0) {
