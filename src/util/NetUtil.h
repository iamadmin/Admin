 ...
 *
 *  Usage:
 *      NetUtil::addrToString(NetUtil::getLocalAddr(NULL))
 *      NetUtil::addrToString(NetUtil::getLocalAddr("eth0"))
 *      NetUtil::isLocalAddr(NetUtil::getAddr("192.168.0.102"))
 */
...
public:
     ...
     */
    static bool isLocalAddr(uint32_t ip, bool loopSkip = false) {
        int fd, intrface;
        struct ifreq buf[16];
        struct ifconf ifc;
